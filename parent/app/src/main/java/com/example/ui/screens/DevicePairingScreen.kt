...
fun DevicePairingScreen(
    ...
    Column(modifier.fillMaxSize().background(DarkBackground).padding(16.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
        TopAppBar(
            title = { Text("Add Child Device", color = TextPrimary, fontWeight = FontWeight.Bold) },
        ...
        Card(Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = DarkSurface)) {
            Column(Modifier.padding(20.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(10.dp)) {
                Text("Real local pairing", color = TextPrimary, fontSize = 19.sp, fontWeight = FontWeight.Bold)
                Text("Show this unique code on the Child device. Game Centre will discover this Parent automatically on the same Wi-Fi network.", color = TextSecondary, fontSize = 12.sp)
                Spacer(Modifier.height(8.dp))
                Text("Unique pairing code", color = TextSecondary, fontSize = 12.sp)
